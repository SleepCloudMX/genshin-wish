#!/usr/bin/env python
"""Character banner plots: CDF, fan, column, rolls2gold."""
...
from genshin_wish.viz._base import setup_style, MISS_LABELS, write_percentile_table
...
def _plot_fan() -> None:
    for interval in [3, 5]:
        ...
        for miss in range(4):
            ...
            plot_luck_fan(
                pdf_func, max_n_up=7, save_path=root / f"miss{miss}.png",
                interval_set=interval, title=f"已连续歪 {miss} 次",
            )

        plot_luck_fan(
            _stable_pdf_func, max_n_up=7, save_path=root / "stable.png",
            interval_set=interval, title="稳态",
        )
